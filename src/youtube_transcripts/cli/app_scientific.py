...
def search_advanced(
    ...
    channels: Optional[str] = typer.Option(None, "--channels", help="Comma-separated channel names"),
    content_type: Optional[str] = typer.Option(None, "--type", help="Content type: lecture, tutorial, conference, demo, discussion"),
    academic_level: Optional[str] = typer.Option(None, "--level", help="Academic level: undergraduate, graduate, research, professional"),
    has_citations: bool = typer.Option(False, "--has-citations", help="Only show videos with citations"),
    institution: Optional[str] = typer.Option(None, "--institution", help="Filter by institution (e.g., MIT, Stanford)"),
    speaker: Optional[str] = typer.Option(None, "--speaker", help="Filter by speaker name"),
    min_quality: Optional[float] = typer.Option(None, "--min-quality", help="Minimum quality score (0-1)"),
    limit: int = typer.Option(10, "--limit", help="Maximum results"),
    export_format: Optional[str] = typer.Option(None, "--export", help="Export format: csv, markdown, json")
):
    ...
    # Perform search
    console.print(f"[cyan]Searching with advanced filters...[/cyan]")
